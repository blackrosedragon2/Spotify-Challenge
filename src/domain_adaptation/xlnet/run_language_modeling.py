...
from dataloader_hf_style import JsonTextDataset
...
def get_dataset(
    ...
    if evaluate:
        return JsonTextDataset(
            data_path=file_path, tokenizer=tokenizer, block_size=1024, split="test"
        )
    else:
        return JsonTextDataset(
            data_path=file_path, tokenizer=tokenizer, block_size=1024, split="training"
        )
