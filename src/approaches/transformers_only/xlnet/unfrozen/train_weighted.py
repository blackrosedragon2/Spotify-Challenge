...
writer = SummaryWriter("transformers_only/xlnet/logs_weighted_unfrozen")
...
def main():
    ...
    parser.add_argument("--patience", type=int, default=4, help="patience")
    ...
    early_stopping = EarlyStopping(
        patience=patience,
        verbose=True,
        path="transformers_only/xlnet/logs_weighted_unfrozen/model/checkpoint"+str(tensorboard_label_number)+".pth",
    )
    ...
    for epoch in range(last_epoch, epochs + 1):
        ...
        while True:
            ...
            y_true += to_label(label_tensor, 0.25)
            y_pred += to_label(output, 0.2)
            ...
            writer.add_scalar(
                "training loss" + tensorboard_label_number + str(learning_rate),
                tr_loss / i,
            ...
            i += 1
        writer.add_scalar(
            "macro f1 score " + tensorboard_label_number + str(learning_rate),
        ...
        if not os.path.exists("transformers_only/xlnet/logs_weighted_unfrozen/model"):
            os.makedirs("transformers_only/xlnet/logs_weighted_unfrozen/model")

        torch.save(
            model.state_dict(),
            "transformers_only/xlnet/logs_weighted_unfrozen/model/ckpt_"
        ...
        while True:
            with torch.no_grad():
                ...
                val_loss += loss.item()
                writer.add_scalar(
                    "validation loss" + tensorboard_label_number + str(learning_rate),
                    val_loss / i,
        ...
        with open(
            "transformers_only/xlnet/output_scores_weighted_unfrozen"
            + str(learning_rate)
            + ".txt",
            "a",
        ) as f:
